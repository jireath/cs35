...
template <typename K, typename V> class HashTable : public Dictionary<K, V> {
  ...
  private:
    LinearDictionary<K,V>* buckets;
    float MaxLoadFactor;
    int size;
    int capacity;
    /**
     * Checks if the current load factor is larger than the MaxLoadFactor. If
     * so, it creates a new buckets array double the current capacity, and
     * rehashes all the keys from the old array into the new array. Deletes 
     * the old array.
     */
    void ensureCapacity();
};
